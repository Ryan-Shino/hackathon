...
def decode_coords(long: float, lat: float) -> str:
    # use mapbox token from file
    mapbox_token = os.environ.get("MAPBOX_TOKEN", TOKEN)

    url = f"https://api.mapbox.com/v4/mapbox.mapbox-streets-v8/tilequery/{long},{
        lat
    }.json"
    ...
    # values converted to lowercase to match the react frontend's state requirements
    # ('exercise', 'learning')
    simplified_types = {
    ...
    # if loop finishes and nothing within 50 metres matches, return default location
    return "centre"
